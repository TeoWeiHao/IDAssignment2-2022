...
$(document).ready(function () {
    ...
    const WORDSDIV = document.querySelector("#wordHistory");
    const SCOREDIV = document.querySelector("#scoreHistory");

    for (let i = 0; i < localStorage.getItem("wordHistory").length; i+=5) {
        var wordP = document.createElement("p");
        wordP.append(localStorage.getItem("wordHistory").slice(i, i+5));
    ...
    function UpdateHistory(){
        var filledList = document.querySelectorAll(".full-correct, .part-correct, .not-correct");

        gameWords = "";
        gameScore = "";

        for (let x of filledList) {
            gameWords += x.querySelector("span").innerHTML;
            if(x.classList.contains("full-correct")){
                gameScore += "🟩";
            }
            else if(x.classList.contains("full-correct")){
                gameScore += "🟨";
            }
            else{
                gameScore += "⬛";
        ...
        localStorage.setItem("wordHistory", gameWords);
        localStorage.setItem("scoreHistory", gameScore);
    }
